		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = false;//변경 전 true
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
    ...
    function html5Upload() {	
    	...
    	sUploadURL= 'file_uploader_html5.php'; 	//upload URL(다중파일 업로더 controller url)
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			...
 			sUrl  : '/upload',	//샘플 URL입니다.(단일 파일업로더 controller url)
 	        sCallback : '/resources/common/editer/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
